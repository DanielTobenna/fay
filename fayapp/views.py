...
def home(request):
	return render(request, 'fayapp/index.html')


def plan(request):
	return render(request, 'fayapp/plan.html')

def otherpayment(request):
    return render(request, 'fayapp/otherpayment.html')


def about(request):
	return render(request, 'fayapp/company.html')


def career(request):
	return render(request, 'fayapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('fayapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'fayapp/login.html')


def privacy(request):
	return render(request, 'fayapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'fayapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('fayapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('fayapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to Metakonet',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('fayapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Metakonet!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@Metakonet.com'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'fayapp/register.html', context)
...
def dashboard(request):
	...
	'client_withdrawal':client_withdrawal, 'client_code':client_code, 'number_of_notifications':number_of_notifications}
	return render(request, 'fayapp/dashboardtwo.html', context)
...
def admindashboard(request):
	...
	return render(request, 'fayapp/adminpage.html', context)


def terms(request):
	return render(request, 'fayapp/terms.html')


def realestate(request):
	return render(request, 'fayapp/realestate.html')


def crypto(request):
	return render(request, 'fayapp/crypto.html')


def forex(request):
	return render(request, 'fayapp/forex.html')


def nft(request):
	return render(request, 'fayapp/nft.html')


def contact(request):
	return render(request, 'fayapp/contact.html')


def stocks(request):
	return render(request, 'fayapp/stocks.html')


def agriculture(request):
	return render(request, 'fayapp/agriculture.html')


def gold(request):
	return render(request, 'fayapp/gold.html')


def retirement(request):
	return render(request, 'fayapp/retirement.html')
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.Metakonet.com/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.Metakonet.com/dashboard"
			})
			...
			template= render_to_string('fayapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@Metakonet.com'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'fayapp/deposittwo.html', context)
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@Metakonet.com'])
	...
	context={}
	return render(request, 'fayapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'fayapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'fayapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'fayapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'fayapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'fayapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'fayapp/referralprofilestwo.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'fayapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('fayapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('fayapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'fayapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	update_payment= client_info.update(deposit=newClientbal, running_days=0, roi=payment_info_investment_plan, investment_plan_name=payment_info_investment_plan_name)

	template= render_to_string('fayapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'fayapp/account_settings.html', context)
...
def notifications(request):
	...
	return render(request, 'fayapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('fayapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('fayapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@Metakonet.com'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'fayapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'fayapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('fayapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('fayapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@Metakonet.com'])
			except BadHeaderError:
				pass
	...
	return render(request, 'fayapp/reinvest.html', context)
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@Metakonet.com'])
	...
	return render(request, 'fayapp/loan.html')